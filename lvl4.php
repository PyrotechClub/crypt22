...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["answer"])) {
        ...
        $log = $answer . " ". $time . "\n";
        fwrite($file, "Side " . $dieside . ", Level 4" . "\n");
        fwrite($file, $log);
        fclose($file);
    }
    
    if ($answer == "popcornexplosion" and $dieside == 3) {
        $levels_solved = $levels_solved . "d";
        date_default_timezone_set('Asia/Kolkata');
        $time = date('Y-m-d H:i:s', time());
        $stmt = $link->prepare("UPDATE users SET points = points + 200, levels_solved = ?, timest = TIMESTAMP($time) WHERE discord_id = ?");
        $stmt -> bind_param("si", $levels_solved, $discord_id);
        // execute the query
        $stmt->execute();
        $stmt->close();
        header('location: lvlselect.php');
    } elseif ($answer == "ithaca" and $dieside == 1) {
        $levels_solved = $levels_solved . "d";
        date_default_timezone_set('Asia/Kolkata');
        $time = date('Y-m-d H:i:s', time());
        $stmt = $link->prepare("UPDATE users SET points = points + 200, levels_solved = ?, timest = TIMESTAMP($time) WHERE discord_id = ?");
        ...
        $stmt->close();
        header('location: lvlselect.php');
    } elseif ($answer == "biden" and $dieside == 2) {
        $levels_solved = $levels_solved . "d";
        date_default_timezone_set('Asia/Kolkata');
        $time = date('Y-m-d H:i:s', time());
        $stmt = $link->prepare("UPDATE users SET points = points + 200, levels_solved = ?, timest = TIMESTAMP($time) WHERE discord_id = ?");
        ...
        $stmt->close();
        header('location: lvlselect.php');
    } elseif ($answer == "aokigahara" and $dieside == 4) {
        $levels_solved = $levels_solved . "d";
        date_default_timezone_set('Asia/Kolkata');
        $time = date('Y-m-d H:i:s', time());
        $stmt = $link->prepare("UPDATE users SET points = points + 200, levels_solved = ?, timest = TIMESTAMP($time) WHERE discord_id = ?");
        ...
        $stmt->close();
        header('location: lvlselect.php');
    } elseif ($answer == "chembarons" and $dieside == 5) {
        $levels_solved = $levels_solved . "d";
        date_default_timezone_set('Asia/Kolkata');
        $time = date('Y-m-d H:i:s', time());
                ...
                <div class="col-md-8">
                    <div class="play-content">
                                ...
                                <div class="inbox">
                                    <div class="tab-content">
                                        <div class="inbox-body">
                                            ...
                                            <b class="right">4 of 4</b>
                                            <div class="inbox-item">
                                                <div class="inbox-item-head">
                                                    <h1 class="lvlno">Level 4</h1>
                                                    <small><i class='fa-regular fa-star'></i> <b>G</b>
                                                        &lt;g@cp.com&gt;</small><br><br>
                                                    <p class="lvltext">time to go ice FISHing</p>
                                                </div>
                                                <div class="inbox-item-body">
                                                    <small>Quick Reply</small><br>
                                                    <small><b>To:</b> &lt;g@cp.com&gt;</small>
                                                    <form method="post"
                                ...
                                <div class="playlist-body">
                                    ...
                                    <div class="playlist-item">
                                        <div class="playlist-item-head">
                                            <h1 class="lvlno">Level 4</h1>
                                            <p class="lvltext">
                                                <img src="lib/goofybeat.png" class="goofybeat">
                                            </p>
                                        </div>
                        ...
                        <div class="play-box">
                            <h1 class="lvlno">Level 4</h1>
                            <?php if($dieside == 1): ?>
                            <p class="lvltext">she changed them to human shape</p>
                            ...
                            <?php if($dieside == 2): ?>
                            <p class="lvltext">
                                uggcf://lbhgh.or/jFDf1b5vkrt<br>
                                <img src="lib/movie.png" alt="movie">
                            </p>
                            <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                                <input type="text" class="form-control" id="answer" name="answer"
                                    placeholder="Enter your answer here"><br>
                            ...
                            <?php if($dieside == 4): ?>
                            <p class="lvltext">
                                <img src="lib/hunger.png" alt="hunger">
                            </p>
                            <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                                <input type="text" class="form-control" id="answer" name="answer"
                                    placeholder="Enter your answer here"><br>
